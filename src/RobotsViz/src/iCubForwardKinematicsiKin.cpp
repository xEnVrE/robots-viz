...
iCubForwardKinematicsiKin::iCubForwardKinematicsiKin(const std::string& part_name) :
    part_name_(part_name)
{
    if ((part_name == "left_hand") || (part_name == "right_hand"))
    {
        ...
        /* The following transformations are required to align the iKin frames to the frames of the meshes. */

        /* Thumb. */

        /* thumb0 to thumb0_r. */
        T thumb0_r = T::Identity();
        thumb0_r.rotate(R(M_PI / 2.0, Vector3d::UnitX()));
        maps_["thumb0"]["thumb0_r"] = thumb0_r;

        /* thumb1 to thumb1_r. */
        T thumb1_thumb1_r = T::Identity();
        thumb1_thumb1_r = Tr(-0.026, 0.0, 0.0);
        thumb1_thumb1_r.rotate(R(M_PI, Vector3d::UnitY()));
        maps_["thumb1"]["thumb1_r"] = thumb1_thumb1_r;

        /* thumb2 to  thumb2_r. */
        T thumb2_thumb2_r = T::Identity();
        thumb2_thumb2_r = Tr(-0.022, 0.0, 0.0);
        thumb2_thumb2_r.rotate(R(M_PI, Vector3d::UnitZ()));
        maps_["thumb2"]["thumb2_r"] = thumb2_thumb2_r;

        /* thumb3 to  thumb3_r. */
        T thumb3_thumb3_r = T::Identity();
        thumb3_thumb3_r = Tr(-0.018, 0.0, 0.0);
        thumb3_thumb3_r.rotate(R(M_PI, Vector3d::UnitY()) * R(-M_PI / 2.0, Vector3d::UnitX()));
        maps_["thumb3"]["thumb3_r"] = thumb3_thumb3_r;

        /* Forward kinematics from iKin. */
        auto finger_names = {"thumb", "index", "middle", "ring", "little"};
        for (const std::string& name : finger_names)
        {
            std::string version = "";
            if (name == "thumb")
                version = "_b";

            std::string laterality = (part_name == "left_hand") ? "left" : "right";

            fingers_fwd_kin_[name] = iCubFinger(laterality + "_" + name + version);
        }


        /* ################################################################ */
        /*  The following are TEMPORARY and to be removed                   */
        /* ################################################################ */
...
iCubForwardKinematicsiKin::~iCubForwardKinematicsiKin()
{}


T iCubForwardKinematicsiKin::map(const std::string& from, const std::string& to, const std::unordered_map<std::string, Eigen::VectorXd>& encoders)
{
    T map;

    std::unordered_map<std::string, yarp::sig::Vector> yarp_encoders;
    for (auto& item : encoders)
    {
        const auto& eigen_encoders = encoders.at(item.first);
        yarp_encoders[item.first].resize(eigen_encoders.size());
        toEigen(yarp_encoders[item.first]) = eigen_encoders;
    }

    auto get_H_as_T = [&](const std::string& name, const int& i)
    {
        auto& finger = fingers_fwd_kin_.at(name);
        finger.setAng(yarp_encoders.at(name));

        T H;
        H.matrix() = toEigen(finger.getH(i));;
    ...
    else if (from == "ee")
    {
        if (to.find("thumb") != to.npos)
        {
            ...
            for (std::size_t i = 0; i < 4; i++)
            {
                ...
                if (to == name_i)
                {
                    map = get_H_as_T(name, i) * maps_.at(name_i).at(name_i + "_r");
                    break;
                }
            }
        }
...
T iCubForwardKinematicsiKin::map(const std::string& from, const std::string& to)
{
...
T iCubForwardKinematicsiKin::DH(const double& d, const double& theta, const double& a, const double& alpha)
{
    T dh;
